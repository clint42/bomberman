...
// Last update Sat Jun 14 14:12:03 2014 virol_g
//
...
bool		MapMenu::build()
{
  ...
  _menuElems.push_back(new MenuButton(std::pair<size_t, size_t>(600, 550),
				      ...
				      glm::vec4(0.51, 0.53, 0.85, 1.f)));
  _titles.push_back(new GraphicalText("Map width", std::pair<size_t, size_t>(400, 180),
				     glm::vec4(0.23, 0.18, 0.52, 1.f), 20, "airstrike"));
  _titles.push_back(new GraphicalText("Map height", std::pair<size_t, size_t>(400, 280),
				     glm::vec4(0.23, 0.18, 0.52, 1.f), 20, "airstrike"));
  _titles.push_back(new GraphicalText("Players on the map", std::pair<size_t, size_t>(400, 380),
				     glm::vec4(0.23, 0.18, 0.52, 1.f), 20, "airstrike"));
  return (true);
}
...
int		MapMenu::getInfo(const std::string &fieldName) const
{
  ...
  for (size_t i = 0; i < _menuElems.size(); ++i)
    {
      if ((tmp = _menuElems[i]->getString()) == fieldName)
	{
	  ss << tmp;
	  ss >> retVal;
	}
